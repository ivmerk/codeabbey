/*ree is a data structure very popular in programming. Just imagine - each web-site usually have some database, each database is usually built of tables, each table usually have one or more indexes - and these indexes are usually implemented as trees.
...
to insert 5 we check the root and since 3 is less, we go to right - this branch is empty, so create new node here with 5 as a value;
