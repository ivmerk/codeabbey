...
answer:
69 56 44
Initially this exercise used angle A instead of B so few of the first submitted solutions may differ slightly. We are sorry for possible inconvenience.
...
for (int i=0; i<number; i++){
     cin >> distance[i] >> angle[i];
     float tmpres;
     tmpres =1.0* distance[i] * tan(PI/180 * (angle[i]-90));
     if (tmpres - (int)tmpres>=0.5)
          res[i] = (int)tmpres+1;
     else 
          res[i] = (int)tmpres;
}
